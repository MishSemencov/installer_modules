...
class STSAction
{
    ...
    public function __construct($action, $data)
    {
        ...
        switch($action){
            ...
            case 'event':
                ...
                break;
			case 'get_tasks_by_group_id':
                $this->getTasksByGroupId($data);
                break;
            case 'get_groups':
                $this->getGroups();
                break;
    ...
    private function createMail($data)
    {
        ...
        if(!$this->getValidateErrors())
        {
            ...
            $eventName = \COption::GetOptionString($this->moduleId, 'sts_mail_type_name');
            $messId = \COption::GetOptionInt($this->moduleId, 'sts_mail_mess_id');
            ...
            if(!empty($eventName) && !empty($messId)){
                ...
                if($res === 'Y') {
                    if ($data["hidden_idea_mode"] == "Y" && Loader::includeModule('ithive.iboard')) {
                        $shortName = \ITHive\IBoard\Ideas::getShortName($data["hidden_idea_id"]);
                        $historyData = Loc::Getmessage("IDEA_MAIL_SENDED", array("#IDEA_ID#" => $data["hidden_idea_id"], "#IDEA_NAME#" => $shortName, "#MAIL_TO#" => $data['email_to']));
                        \ITHive\IBoard\History::add($historyData, $USER->GetId(), $data["hidden_idea_id"], false, "mail");
                    }
                    $this->status = true;
                }
    ...
    {
        $requiredParams = ['title', 'description', 'responsible_id', 'parent_task_id', 'group_id'];
        $this->validateParams($requiredParams, $data);
        if(!$this->getValidateErrors())
        {
			...
			if (!empty($data['parent_task_id']))
                $arFields['PARENT_ID'] = $data['parent_task_id'];
            ...
            if($taskId>0){
                ...
                if ($data["hidden_idea_mode"] == "Y" && Loader::includeModule('ithive.iboard')) {
                    \ITHive\IBoard\History::add($taskUrl, $USER->GetId(), $data["hidden_idea_id"], false, "task");
                }
            }
        ...
        if(!$this->getValidateErrors())
        {
            ...
            if($messId) {
                if ($data["hidden_idea_mode"] == "Y" && Loader::includeModule('ithive.iboard')) {
                    ...
                    \ITHive\IBoard\History::add($historyData, $USER->GetId(), $data["hidden_idea_id"], false, "chat");
                }
                $this->status = true;
            }
        }
    ...
    private function createLFMessage($data)
    {
        ...
        if(!$this->getValidateErrors())
        {
            ...
            if(empty($arBlog['ID'])){
                $arBlogFields = array(
                    "NAME" => Loc::getMessage('USER_BLOG_TITLE').' '.$USER->GetFullName(),
                    "GROUP_ID" => '1',
                    "ENABLE_IMG_VERIF" => 'Y',
                    "EMAIL_NOTIFY" => 'Y',
                    "ENABLE_RSS" => "Y",
                    "USE_SOCNET" => "Y",
                    "URL" => $USER->GetLogin() . "-blog",
                    "ACTIVE" => "Y",
                    "OWNER_ID" => $USER->GetID()
                );
                \CBlog::Add($arBlogFields);
                $arBlog = \CBlog::GetByOwnerID($userId);
            }
            $arFields = array(
                "TITLE" => ' ',
            ...
            if($messId>0){
                ...
                if ($data["hidden_idea_mode"] == "Y" && Loader::includeModule('ithive.iboard')) {
                    ...
                    \ITHive\IBoard\History::add($historyData, $USER->GetId(), $data["hidden_idea_id"], false, "lfeed");
                }
    ...
    private function createEvent($data)
    {
        ...
        if(!$this->getValidateErrors())
        {
            ...
            if($eventId>0){
                ...
                if ($data["hidden_idea_mode"] == "Y" && Loader::includeModule('ithive.iboard')) {
                    \ITHive\IBoard\History::add($eventUrl, $USER->GetId(), $data["hidden_idea_id"], false, "event");
                }
            }
	...
	private function getTasksByGroupId($data)
    {
...
//                    'GROUP_ID' => $data['group_id']
            ...
            $this->status = $arTasks?true:false;
            $this->resultData = [
                'tasks' => $arTasks?$arTasks:[]
            ];
    ...
    private function getGroups()
    {
        ...
        $filter = array("ACTIVE" => "Y", "VISIBLE" => "Y");
        $dbGroup = \CSocNetGroup::GetList(array("NAME" => "DESC"), $filter, false, false, array());
        while($arGroup = $dbGroup->Fetch()) {
            $arGroups = array(
                "id" => $arGroup["ID"],
                "name" => $arGroup["NAME"],
                "desc" => $arGroup["DESCRIPTION"],
                "image" => ($arGroup["IMAGE_ID"]) ? CFile::GetPath($arGroup["IMAGE_ID"]) : false
            );
        }
